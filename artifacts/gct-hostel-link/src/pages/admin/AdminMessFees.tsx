...
import { Loader2, CheckCircle, XCircle, Search, DollarSign, Edit2, Download, AlertTriangle, Save } from "lucide-react";
...
export default function AdminMessFees() {
  ...
  function handleExport() {
    const rows = filtered.map((s) => ({
      ...
      "Paid At": fees[s.id]?.paid_at ? formatDate(fees[s.id]!.paid_at!) : "",
    }));
    exportToCSV(rows, `admin_mess_fees_${month}`);
  }
  ...
  async function markPayment(student: Profile, amountStr: string) {
    ...
    if (isNaN(amount) || amount <= 0) { setPayAmountError("Amount must be greater than 0."); return; }
    if (amount > 999999) { setPayAmountError("Amount seems too high. Please verify."); return; }
    setPayAmountError("");
    setSubmitting(student.id);
    const { data: { user } } = await supabase.auth.getUser();
    let error;
    if (fees[student.id]?.id) {
      const res = await supabase.from("mess_fees").update({ status: "paid", amount, paid_at: new Date().toISOString(), marked_by: user?.id }).eq("id", fees[student.id].id);
      error = res.error;
    } else {
      const res = await supabase.from("mess_fees").insert({ student_id: student.id, month, amount, status: "paid", paid_at: new Date().toISOString(), set_by: user?.id, marked_by: user?.id });
      error = res.error;
    }
    if (error) {
      toast({ title: "Payment Failed", description: "Could not save payment. No changes were made.", variant: "destructive" });
    } else {
      await supabase.from("audit_logs").insert({ table_name: "mess_fees", record_id: fees[student.id]?.id || student.id, field_name: "status", old_value: fees[student.id]?.status || "unpaid", new_value: "paid", changed_by: user?.id });
  ...
  async function confirmReversePayment() {
    ...
    setReversing(true);
    const { data: { user } } = await supabase.auth.getUser();
    const { error } = await supabase.from("mess_fees").update({ status: "unpaid", paid_at: null, marked_by: user?.id }).eq("id", fee.id);
    if (error) {
      toast({ title: "Reversal Failed", description: "Could not reverse payment. Please try again.", variant: "destructive" });
    } else {
      await supabase.from("audit_logs").insert({ table_name: "mess_fees", record_id: fee.id, field_name: "status", old_value: "paid", new_value: "unpaid", changed_by: user?.id });
      toast({ title: "Payment Reversed", description: `${student.name}'s payment reversed to unpaid.` });
    }
    setReversing(false);
  ...
  async function saveIndividualAmount(studentId: string) {
    ...
    if (isNaN(amt) || amt <= 0) { toast({ title: "Invalid amount", description: "Amount must be greater than 0.", variant: "destructive" }); return; }
    ...
    if (error) {
      ...
      if (fee?.id) await supabase.from("audit_logs").insert({ table_name: "mess_fees", record_id: fee.id, field_name: "amount", old_value: String(fee.amount), new_value: String(amt), changed_by: user?.id });
  ...
  const filtered = students.filter((s) => {
    ...
    return matchSearch && (filterStatus === "all" || feeStatus === filterStatus);
  });
  ...
  return (
    <div>
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
        <div>
          <h1 className="text-2xl font-bold text-foreground">Mess Fees (Admin)</h1>
          <p className="text-sm text-muted-foreground">Full control — set amounts, mark paid, reverse, all logged</p>
        </div>
        <div className="flex gap-2">
          <input type="month" value={month} onChange={(e) => setMonth(e.target.value)}
            className="border border-input rounded-lg px-3 py-2 text-sm bg-background text-foreground" />
          <Button onClick={handleExport} variant="outline" size="sm"><Download className="w-4 h-4 mr-1" />CSV</Button>
        </div>
      </div>
                  ...
                  <div className="flex items-center justify-between gap-3 flex-wrap">
                    <div className="flex items-center gap-3 min-w-0">
                      ...
                      <div className="min-w-0">
                        ...
                        {fee?.paid_at && <div className="text-xs text-muted-foreground">Paid: {formatDate(fee.paid_at)}</div>}
                      </div>
                    </div>
      ...
      <Dialog open={!!paymentModal} onOpenChange={(open) => { if (!open) { setPaymentModal(null); setPayAmountError(""); } }}>
        <DialogContent className="max-w-sm">
          ...
          {paymentModal && (
            <div className="space-y-4 py-2">
              ...
              <div>
                ...
                <Input className={`mt-1.5 ${payAmountError ? "border-red-500" : ""}`} type="number" min="1"
                  value={payAmount} onChange={(e) => { setPayAmount(e.target.value); setPayAmountError(""); }} placeholder="Enter amount in PKR" />
          ...
          <DialogFooter>
            ...
            <Button onClick={() => paymentModal && markPayment(paymentModal, payAmount)}
              className="bg-gradient-to-r from-green-500 to-emerald-600 text-white" disabled={submitting === paymentModal?.id}>
        ...
        <DialogContent className="max-w-sm">
          ...
          {reverseModal && (
            <div className="space-y-3 py-2">
              ...
              <p className="text-sm text-muted-foreground">This will mark the payment as <strong className="text-red-600">unpaid</strong>. This action is logged in audit records.</p>
            </div>
          )}
          <DialogFooter>
