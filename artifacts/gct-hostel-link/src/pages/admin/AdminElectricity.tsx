...
export default function AdminElectricity() {
  ...
  const filtered = students.filter((s) => {
    ...
    const status = bills[s.id]?.status || "unpaid";
    const matchStatus = filterStatus === "all" || status === filterStatus;
    return matchSearch && matchStatus;
  });

  const paidCount = students.filter(s => bills[s.id]?.status === "paid").length;
  const unpaidCount = students.filter(s => bills[s.id]?.status !== "paid").length;
  ...
  return (
    <div>
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
        <div>
          <h1 className="text-2xl font-bold text-foreground">Electricity Bills (Admin)</h1>
          <p className="text-sm text-muted-foreground">Full control — set, update, and mark payments</p>
        </div>
      ...
      <div className="flex flex-col sm:flex-row gap-3 mb-4">
        <Input placeholder="Search by name or roll no..." className="flex-1" value={search} onChange={(e) => setSearch(e.target.value)} />
      ...
      {loading ? (
        ...
        <div className="space-y-2">
          {filtered.length === 0 && <div className="text-center py-12 text-muted-foreground">No records found</div>}
          {filtered.map((s) => {
                    ...
                    <div className="flex items-center gap-2 flex-shrink-0 flex-wrap justify-end">
                      ...
                      {bill && (
                        <>
                          ...
                          <Button size="icon" variant="ghost" className="h-8 w-8" title={bill.status === "paid" ? "Reverse to unpaid" : "Mark as paid"} onClick={() => setToggleModal({ student: s, bill })}>
